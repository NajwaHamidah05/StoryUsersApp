...
import com.faradhy.storyusersapp.data.database.RemoteKeys
...
class StoryRemoteMediator(
...
) : RemoteMediator<Int, RowItemStory>() {
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, RowItemStory>
    ): MediatorResult {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, RowItemStory>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            storyDatabase.remoteKeysDao().getRemoteKeysId(data.id)
        }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, RowItemStory>): RemoteKeys? {
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()?.let { data ->
            storyDatabase.remoteKeysDao().getRemoteKeysId(data.id)
        }
    }

    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, RowItemStory>): RemoteKeys? {
